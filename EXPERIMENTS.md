...
> **Relationship to other docs.** `MATHEMATICAL_REFERENCE.md` derives the RL/EA/energy math in depth and is the reference for the *equations*; it has been brought up to the same implementation this file documents (tournament EA, on‑policy REINFORCE with baseline and traces, 54→128→6 network, mutation enabled), so the two now agree on every constant. This file remains authoritative for **experiment and run structure** — what was run, on which seeds, with which scripts. `HEADLESS.md` covers the headless runner in general; here we focus on the experiment set.
...
|-----------|-----------|----------------|---------------|------|
| **Evolution** | Evolutionary algorithm only — no within‑life learning | `genome_weights` | n/a (no learning) | `GAManager` (`--condition evolution`) |
| **Learning** | EA **plus** in‑lifetime REINFORCE | `genome_weights` | **No** — RL adapts `active_weights` within a life for behaviour, but only the frozen genome is selected/inherited; learned drift dies with the organism (a Baldwin‑effect setup) | `GAManager` with `rl_enabled` (`--condition learning`) |
| **Pure RL** | In‑lifetime REINFORCE only — no EA | `genome_weights` within a generation, `active_weights` across one | **Between generations only** — learned weights are synced into the genome at the window boundary and reseed the next window, so learning persists and propagates across generations but not from a parent to its within‑life offspring (§3.3) | `PureRLManager` (`--mode pure_rl`) |
...
* **Only the terrain is seeded.** EA mutation, RL exploration, reproduction chance and predator wandering all use unseeded `Math.random()` — deliberately, so each run is an independent sample of stochastic dynamics on a fixed world.
...
**Fitness = `cumulative_food_score`** — the total food value an organism consumed over its life. This is the EA's selection signal and is **independent of the RL reward**, so learning and evolution are compared on the same yardstick. Prestige food is worth the most but is guarded by patrol predators when those are enabled.
...
* **Inheritance depends on condition** (this is the crux):
  * **EA conditions (evolution, learning):** children inherit the parent's **`genome_weights` only** (non‑Lamarckian) plus a small asexual mutation (`ASEXUAL_MUT_PROB = 0.05`, `σ = 0.1`). Any RL drift in the parent's `active_weights` is **not** passed on.
  * **Pure RL:** identical to the EA conditions *within* a generation — the child inherits the parent's `genome_weights` plus the same asexual mutation, and the parent's RL drift is **not** passed on. Learned weights are inherited only at the window boundary (§3.3).
...
Identical EA machinery to §3.1, **plus** every organism runs **REINFORCE with eligibility traces** each tick, updating its `active_weights` (not the genome). The genome stays frozen through life and is the only thing the EA selects/inherits — so learning shapes *behaviour and survival* within a life, but evolution still searches genome space (Baldwin effect, **non‑Lamarckian**).
...
REINFORCE only — **no EA, no fitness‑based selection**. Adaptation comes from (1) within‑life RL, (2) energy‑triggered reproduction, and (3) an undirected between‑episode mutation. Population management at each 10 000‑tick window boundary (`closeWindow()`):
...
**Where the Lamarckian step actually happens — read this before describing the
condition.** Learning enters the heritable genome **only at the window
boundary**, by two routes: a survivor's `active_weights` are mutated and copied
into its `genome_weights` (`PureRLManager.closeWindow`), and the top‑up samples
`active_weights` snapshots of dead organisms from the buffer
(`_bufferDeadWeights`). **Within** a generation the condition is
non‑Lamarckian, exactly like the EA arms: `AdvancedOrganism.reproduce()` builds
the child from `this.brain.getGenome()`, and `getGenome()` returns
`genome_weights`, which is fixed at the parent's own birth value for its whole
life.

`PureRLManager.registerAgent` carries a `syncGenomeFromActive()` call intended
to make within‑generation reproduction Lamarckian, but it never executes: it is
guarded by `if (agent.parent && ...)` and `this.parent` is never assigned
anywhere in the codebase — `Organism` and `AdvancedOrganism` take `parent` as a
constructor argument and use it for `inherit()` and `brain.copy()` without
storing it. It is also sequenced after `child.brain.setGenome(...)`, so even
with the reference wired up the child would already have been built from the
pre‑sync genome and only the parent's *next* child could benefit.

This is documented rather than fixed because every reported run was produced by
the code as it stands; changing the mechanism would invalidate all 100 pure‑RL
runs per environment. The condition is still the arm with no fitness‑based
selection, and learning still persists and propagates across windows — the
correction is to the timing of the Lamarckian step, not to what the condition
is.
...
| **Inter‑generation** | EA between‑gen mutation; pure‑RL between‑episode + top‑up | 0.03 / 0.1 | `ExperimentParams.mut_prob/mut_sigma` (honours `--mut-prob/--mut-sigma`) |

**How "identical" is verified.** The inter‑generation kernel is written out *twice* — `GAManager._mutate` for the two EA conditions and `PureRLManager._mutateWeights` for pure RL — so identity is a property that has to be measured, not read off the call graph. `node src/eval/validate.js --only mut` measures it three ways:
...
* **Natural disasters** (`--disaster …`) — an optional mass‑mortality event that culls a fixed fraction of the selection pool before EA selection, on random generations (Bernoulli `disaster_prob`), driven by a dedicated reproducible PRNG. A gradual‑recovery variant tapers the culled fraction over generations. Off in the core comparison; swept by the disaster experiments.
...
|--------|-----------------|-----------------|
| `slurm/run_evolution_condition_hard_w500_h128_array.slurm` | `--condition evolution --mode standard --log-genomes` | `roam60d5_w500_evolution_h128_seed<S>_r<R>` |
| `slurm/run_learning_condition_hard_w500_h128_array.slurm` | `--condition learning --mode standard --no-epsilon --learning-rate 0.02 --log-genomes` | `roam60d5_w500_learning_h128_lr0.02_seed<S>_r<R>` |
| `slurm/run_pure_rl_condition_hard_w500_h128_array.slurm` | `--condition learning --mode pure_rl --no-epsilon --learning-rate 0.02` | `roam60d5_w500_purerl_h128_noeps_lr0.02_seed<S>_r<R>` |
| `slurm/run_evolution_condition_baseline_w500_h128_array.slurm` | as above, `--roaming-predators 0 --predators-per-patch 0` | `base_w500_evolution_h128_seed<S>_r<R>` |
| `slurm/run_learning_condition_baseline_w500_h128_array.slurm` | as above but `--learning-rate 0.01` | `base_w500_learning_h128_lr0.01_seed<S>_r<R>` |
| `slurm/run_pure_rl_condition_baseline_w500_h128_array.slurm` | as above but `--learning-rate 0.01` | `base_w500_purerl_h128_noeps_lr0.01_seed<S>_r<R>` |

The two RL‑using conditions share identical hyperparameters within an environment, so
**learning vs pure‑RL isolates the EA's contribution** and **learning vs evolution isolates
...
`slurm/run_random_floor_condition_hard_w500_h128_array.slurm`: the same 10 seeds, 3 replicates,
...
```bash
sbatch slurm/run_learning_condition_hard_w500_h128_array.slurm
GENERATIONS=500 sbatch slurm/run_evolution_condition_hard_w500_h128_array.slurm
SEEDS="2001 2002 2003" N_REPS=10 sbatch --array=0-29 slurm/run_pure_rl_condition_hard_w500_h128_array.slurm
```
...
* `run_lr_sweep_learning_w1k_array.slurm` — learning condition (EA + RL, no epsilon), `learning_rate ∈ {0.01, 0.02, 0.03}` × {baseline, hard} × 3 seeds (`1 42 999`) × 5 replicates = **90 runs**, one per array task (~58 h baseline / ~44 h hard; 90 h wall clock, 24 G).
...
* Job script: `slurm/run_random_floor_condition_hard_w500_h128_array.slurm` (same 10 seeds, same environment, same 100‑run grid as the trio).
...
* **Unseeded (deliberate):** EA mutation, RL exploration, reproduction chance, predator wandering. Multiple seeds × the across‑seed aggregation in the analysis scripts capture this stochasticity.
