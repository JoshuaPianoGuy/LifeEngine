"""
analyse_learning_hard_runs.py
======================================================================
The LEARNING campaign on the HARD environment, read as one population of
independent runs (the 100-run set: 10 map seeds x 10 replicates).

The sibling of analyse_evolution_hard_runs.py, drawing the SAME figures off the
SAME columns for the other arm of the matched trio, so the two sets of PNGs can
be put side by side without a single axis or colour differing. Like that script
...
Runs are found by params.json (never by folder name):
    ...
    condition                 == learning
    mode                      == standard        <- SEE BELOW, THIS ONE MATTERS
...
WHY --mode EXISTS HERE AND NOT IN THE EVOLUTION SCRIPT
------------------------------------------------------
The pure-RL arm is ALSO logged with condition == "learning": its job script
passes `--condition learning --mode pure_rl`, because --condition switches
in-lifetime RL on and --mode chooses whether the GA runs at all. So under
logs/learning/ the string "learning" alone does NOT identify this campaign —
it currently matches 160 runs (100 standard + 60 pure_rl), and pooling them
would silently average two different algorithms into one mean curve.
`--mode standard` (the default) is what keeps this to the 100 GA+RL runs.
Pass `--mode pure_rl` to point the same figures at the pure-RL arm instead, or
`--mode any` to deliberately pool. The evolution script needs none of this
because nothing else is ever logged as condition == "evolution".

Written, into --out (default output/learning_hard_runs):

    learning_hard_fitness.png
    ...
    learning_hard_overview.png
        the four-panel headline figure (fitness, population, genome variance,
        final-fitness strip).
    learning_hard_weight_space.png
        THE ONE FIGURE WITH NO EVOLUTION COUNTERPART, in three panels: the
        phenotype's in-life drift (avg_learned_weight_diff, a mean absolute weight difference), the genome's
        weight LEVEL (avg_network_weight_mag, an RMS) and that level's RATE of
        change (inter_gen_weight_change). See the RL_METRICS comment below for
        what each one is and why the level and the rate must be read together.
        In the evolution runs avg_learned_weight_diff is identically 0.0 — there
        is no in-lifetime learning to measure — so this cannot be part of a
        shared figure set. Its other two panels DO have an evolution
        counterpart — evolution_hard_weight_space.png — drawn in the same colours
        with the same axis labels for side-by-side reading. Suppress with
        --no-weight-space.
    learning_hard_runs.csv
        per-run endpoint table -- one row per run, the data behind the strip.
    learning_hard_curves.csv
...
mutation, the RL policy's own sampling and predator wandering are unseeded, so
the 10 runs of a seed are independent samples. `--reps N` truncates every seed to
its first N runs (folder suffix _r<N> orders them), so the same campaign can be
re-analysed as a 3- or 5-run one and the figures compared like for like.
...
Usage
-----
    python analyse_learning_hard_runs.py

    # the pure-RL arm through the identical figures:
    python analyse_learning_hard_runs.py --mode pure_rl --out output/purerl_hard_runs

    # band = between-seed spread instead of between-run:
    python analyse_learning_hard_runs.py --aggregate seeds

    # median/IQR instead of mean/std; the literal last generation as "final":
    python analyse_learning_hard_runs.py --band iqr --final-window 1

    # re-analyse the 10-per-seed campaign as a 3-per-seed one:
    python analyse_learning_hard_runs.py --reps 3 --out output/learn_hard_reps3
"""
...
# ── Colours ───────────────────────────────────────────────────────────────────
# IDENTICAL to analyse_evolution_hard_runs.py, deliberately: a metric wears the
# same hue in both arms so the two figure sets can be read side by side. Every
# set that shares a panel was checked with the palette validator for
...
# genome variance and the two in-life learning series are lone series on their
# own panels, so they only have to clear the surface.
...
METRIC_COLOURS = {
    ...
    'avg_learned_weight_diff': '#059669',   # green  (learning arm only)
...
# than 4.375. Everything measured against this line is therefore OPTIMISTIC —
# but uniformly so, which is why it stays usable for comparing arms.
#
# THE SAME 4.375 IS USED ON THE LANDSCAPE PROBES TOO — see
# landscape/plot_slices.py resolve_replacement(). An earlier version of this
# comment claimed the probe scale needed a "k conversion" first; that is wrong on
# both counts. Reproduction is energy-neutral in this simulator
# (AdvancedOrganism.reproduce() deducts nothing, 3.5 is a counter threshold), so
# running a probe with reproduction off does not move f onto another scale. And
# the two k estimates in the Stage-9 calibration disagree by a factor of two
# because one of them divides by a replay mean taken over a mixed-age population;
# neither is a conversion factor. Stage 9's measured self-replacement crossing
# (f = 5.39 for the hard environment) is a real alternative to 4.375, but it is a
# per-environment measurement resting on two genomes, so it is kept as an opt-in
# sensitivity check rather than the default.
...
METRICS = {
    ...
    'avg_learned_weight_diff': ('In-life mean absolute weight difference', '{:.5f}'),
...
# The weight-space panel, ordered PHENOTYPE, LEVEL, RATE — and the order matters,
# because the third is the derivative of the second and is easy to misread on its
# own:
#   avg_learned_weight_diff  mean absolute weight difference of active vs genome weights: how far a lifetime
#                            of RL moves the phenotype off the inherited genome.
#                            Identically 0 without RL.
#   avg_network_weight_mag   RMS of the GENOME weights, ||w||/sqrt(N). The LEVEL:
#                            the typical size of one heritable weight. Starts at
#                            the Xavier init value (~0.106 at h128) and is capped
#                            at 1.0 by the mutation clamp in GAManager._mutate.
#   inter_gen_weight_change  the FIRST DIFFERENCE of the line above — a RATE, not
#                            a magnitude. It decaying to 0 means the genome's
#                            weight scale has PLATEAUED (here at ~0.61), NOT that
#                            the weights vanished. Plotted directly beneath the
#                            level so the two are read together; cumulatively
#                            summing it reproduces the level exactly.
RL_METRICS = ['avg_learned_weight_diff', 'avg_network_weight_mag',
...
TABLE_METRICS = FITNESS_METRICS + POP_METRICS + [VARIANCE_METRIC,
                                                 'avg_lifetime', 'avg_energy_end'] + RL_METRICS
...
def discover(logs_root, want, seeds=None, reps=None):
    ...
    the condition/mode/environment filters. params.json is authoritative — folder
    names are only a hint. Returns a list of dicts sorted by (seed, rep):
    {seed, rep, name, dir, gen_csv}.

    `want` keys: condition (exact), mode (exact, skipped if None — this is what
    separates the GA+RL runs from the pure-RL ones, which share a condition),
    roaming (tol 1e-9), drain (tol 1e-9, skipped if None). `seeds` keeps only
    those map seeds; `reps` keeps only the first N runs of each seed.

    RANDOM-FLOOR RUNS ARE EXCLUDED BY DEFAULT. The chance-anchor runs
    (--random-floor, see ExperimentParams.random_floor) record
    condition == "evolution" and mode == "standard" in params.json, because that
    is what they are minus between-generation selection — so without this filter
    they would be pooled straight into the evolution arm and drag its mean down
    to chance. Pass want['random_floor'] = True to select them INSTEAD. Runs
    predating the flag have no such key, which reads as False, the right default."""
    runs = []
    for params_path in glob.glob(os.path.join(logs_root, '**', 'params.json'),
        ...
        if str(p.get('condition')) != want['condition']:
            continue
        # Missing 'mode' means a run predating the pure-RL split; those are all
        # standard, so treat absence as 'standard' rather than dropping them.
        if want.get('mode') is not None and str(p.get('mode', 'standard')) != want['mode']:
            continue
        # See the docstring: the chance anchor masquerades as an evolution run.
        if bool(p.get('random_floor', False)) != bool(want.get('random_floor', False)):
            continue
...
def main():
    ap = argparse.ArgumentParser(
        description='Mean curves and per-run endpoints for the learning '
                    '(GA + in-lifetime RL) campaign on the hard '
                    '(roaming-predator) environment.')
    ap.add_argument('--logs-root', default='logs/learning',
                    help='Root to search recursively for run folders. '
                         'Default: logs/learning.')
    ap.add_argument('--out', default=None,
                    help='Output directory. Default: output/learning_<env>_runs.')
    ...
    ap.add_argument('--condition', default='learning',
                    help='params.json condition to match. Default: learning.')
    ap.add_argument('--mode', default='standard',
                    help='params.json mode to match: standard (GA + RL, the '
                         'default), pure_rl (RL only — logged under the SAME '
                         'condition, so without this filter both arms would be '
                         'pooled), or "any" to disable the filter.')
    ...
    # --no-rl-metrics is the original spelling, kept as an alias so anything
    # already scripted against it keeps working; --no-weight-space is the name
    # shared with analyse_evolution_hard_runs.py.
    ap.add_argument('--no-weight-space', '--no-rl-metrics', action='store_true',
                    dest='no_weight_space',
                    help='Skip the weight-space figure, for strict '
                         'figure-for-figure parity with the evolution output.')
    ...
    want = {'condition': args.condition, 'mode': mode}
    if args.env == 'hard':
        want['roaming'], want['drain'] = args.predators, args.drain
    else:
        want['roaming'], want['drain'] = env['roaming'], env['drain']

    # The mode is part of the identity of this campaign, so it is part of the
    # default output path too — otherwise `--mode pure_rl` would quietly
    # overwrite the standard arm's figures.
    default_out = f'output/learning_{args.env}_runs'
    if mode not in (None, 'standard'):
        default_out = f'output/learning_{mode}_{args.env}_runs'
    out_dir = args.out or default_out
    os.makedirs(out_dir, exist_ok=True)

    mode_desc = 'any mode' if mode is None else f'mode={mode!r}'
    print(f'Searching {args.logs_root} for condition={args.condition!r}, '
          f'{mode_desc} in the {args.env} environment ({env["desc"]}) ...')
    runs = discover(args.logs_root, want, set(args.seeds) if args.seeds else None,
                    args.reps)
    if not runs:
        print('No matching runs found. Check --logs-root / --env / --condition / --mode.')
        return
    ...
    stem = f'learning_{args.env}' if mode in (None, 'standard') else f'learning_{mode}_{args.env}'

    runs_csv = os.path.join(out_dir, f'{stem}_runs.csv')
    table.to_csv(runs_csv, index=False)
    print(f'\n  wrote {runs_csv}')

    tidy = curves_table(curves, args.aggregate, args.band)
    curves_csv = os.path.join(out_dir, f'{stem}_curves.csv')
    ...
    arm = args.condition if mode in (None, 'standard') else f'{args.condition}/{mode}'
    # The headline title names the ALGORITHM, not the log tree it lives in.
    # --mode pure_rl is still logged as condition == "learning" (the flag turns
    # in-lifetime RL on; --mode chooses whether the GA runs at all), so titling
    # it "Learning" off the condition alone would label the GA-free arm as the
    # GA+RL one.
    arm_title = {'pure_rl': 'Pure RL'}.get(mode, args.condition.capitalize())
    subtitle = (f'{arm} · {env["desc"]} · {len(table)} runs '
    ...
    plot_curve_figure(curves, FITNESS_METRICS, args.aggregate, args.band,
                      ...
                      os.path.join(out_dir, f'{stem}_fitness.png'))
    plot_curve_figure(curves, POP_METRICS, args.aggregate, args.band,
                      args.smooth, show_runs,
                      f'Population over generations — {subtitle}',
                      os.path.join(out_dir, f'{stem}_population.png'))
    plot_curve_figure(curves, [VARIANCE_METRIC], args.aggregate, args.band,
                      args.smooth, show_runs,
                      f'Genome variance over generations — {subtitle}',
                      os.path.join(out_dir, f'{stem}_genome_variance.png'))
    plot_strip_figure(table, args.final_window,
                      os.path.join(out_dir, f'{stem}_final_fitness_strip.png'),
                      args.jitter_seed)
    plot_overview(curves, table, args.aggregate, args.band, args.smooth,
                  show_runs, args.final_window,
                  f'{arm_title} on the {args.env} environment — {subtitle}',
                  os.path.join(out_dir, f'{stem}_overview.png'),
                  args.jitter_seed)
    if not args.no_weight_space:
        plot_curve_figure(curves, RL_METRICS, args.aggregate, args.band,
                          args.smooth, show_runs,
                          f'Weight space: in-life drift, genome level, and its '
                          f'rate of change — {subtitle}',
                          os.path.join(out_dir, f'{stem}_weight_space.png'))
