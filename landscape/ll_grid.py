...
def reconstruct_field(results_df, axes, rl, column):
    """Return an (resolution, resolution) grid of ANY per-probe column, averaged
    over the R repeats.

    reconstruct_grid() is hard-wired to mean_fitness (plus its std). Stage 9
    needs mean_lifetime on the same mesh — the difficulty metrics relate the
    learning lift to how long agents survive — and other columns
    (mean_predator_touches, mean_cave_entries) are equally available.
    """
    N = axes['resolution']
    tag = 'on' if rl else 'off'
    ...
    df = df[df['tag'] == tag]
    for (i, j), sub in df.groupby(['i', 'j']):
        out[int(i), int(j)] = sub[column].to_numpy().mean()
    return out
